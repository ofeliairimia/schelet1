...
const addItemToCart = (name, price, imageSrc) => {
  ...
  cartItem.innerHTML = `
      ...
      <div class="item-details">
        ...
        <div>
            <p>
                ${price.toFixed(2)} RON x ${count} 
                <span class='total-price'>${totalPrice.toFixed(2)} RON</span>
            </p>
        </div>
        </div>
...
checkout.addEventListener("click", () => {
  const phoneNumber = '+40761517068'; // Înlocuiește numărul de WhatsApp

  // Construiește mesajul automat cu produsele din coș
  let message = 'Bună ziua, aș dori să finalizez comanda cu următoarele produse:%0A';

  const cartItemsList = document.querySelectorAll(".cart-item");
  cartItemsList.forEach((item) => {
    const productName = item.querySelector(".item-details > div").textContent.trim();
    const quantity = item.dataset.quantity;
    const totalPrice = item.querySelector('.total-price').textContent.trim();

    message += `- ${productName},  Cantitate: ${quantity},  Total: ${totalPrice}%0A`;
  });

  // Adaugă un mesaj suplimentar dacă este necesar (de exemplu, adresa de livrare)
  message += '%0AAdresa de livrare: [ INTRODU AICI ADRESA ]';
